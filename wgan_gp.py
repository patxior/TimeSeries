...
# READ DATA
x_train, y_train, x_test, y_test = utilis.load_UCR_data(folder='ItalyPowerDemand', xy_split=True, plot=True, expand_dims_for_conv=True)
train_images = x_test
...
class GANMonitor(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for i in range(self.num_img):
            ...
            plt.plot(img.squeeze(), color='red', alpha=0.2)
            # img = keras.preprocessing.image.array_to_img(img)
            # img.save("generated_img_{i}_{epoch}.png".format(i=i, epoch=epoch))
        plt.plot(
            train_images[np.random.choice(train_images.shape[0], size=12, replace=False)].squeeze().transpose(),
            color='green',
            alpha=0.2,
            )     
        plt.show()  
